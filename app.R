...
ui <- fluidPage(
  theme = shinytheme("cerulean"),
    ...
    ")),
   # Application title
   titlePanel("Cli-mates modeller"),
   
   # Sidebar with a slider input for number of bins 
   sidebarLayout(
      sidebarPanel(
         h3("Set 2 layer model parameters"),
         ...
         #numericInput("w_d", "Water density"),
         numericInput("gamma", "Gamma", value = 1.05),
         #numericInput("v_d", "Vertical diffusivity"),
         #numericInput("c_w", "Heat capacity of water"),
         ...
         hr(),
         h3("Select year"),
         sliderInput("Year", label  = "Single entity climate model", min = 1850, max = 2500, value = 1850, step = 65, animate = TRUE, sep = "", ticks = TRUE),
         sliderInput("Year_double", label  = "2 layer climate model", min = 1850, max = 2500, value = 1850, step = 65, animate = TRUE, sep = "", ticks = TRUE),
         tableOutput("done")
      ),
...
server <- function(input, output) {
  ...
  #values <- data.frame("Temperature" = 1, "Year" = 2)
  new_filtered <- data.frame("Temperature" = 1, "Year" = 2)
 ...
 # two layer model
  ...
  observeEvent(input$run, {
    ...
    output$done <- renderTable(values)
  })
  ...
  observeEvent(input$plot, {
    ...
    output$climateplot <- renderPlot({
      ...
      ggplot() + #geom_path(aes(x = Year, y = Temperature), data = filtered, stat  = "identity", size = 1.5) +
        geom_point(aes(x = Year, y = Temperature), data = values, size = 1.5, colour = "black", na.rm = TRUE) + 
        #scale_x_continuous(limits = c(1850, 2500), breaks = c(1850, 1900, 1950, 2000, 2050, 2100, 2150, 2200, 2250, 2300, 2350, 2400, 2450, 2500)) + 
